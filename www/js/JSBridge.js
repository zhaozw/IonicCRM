...
(function () {
	...
	if (typeof MobileCRM === "undefined") {
		MobileCRM = {
			...
			UI: {
				...
				LookupForm: function () {
				    ...
				    /// <field name="allowedViews" type="String">Allowed views, or null if all are allowed.</field>
		...
		MobileCRM.UI.LookupForm.prototype.addView = function (entityName, viewName, isDefault) {
		    /// <summary>Create structure for allowed views.</summary>
		    /// <param name="entityName" type="string">name of entity.</param>
		    /// <param name="viewName" type="string">name of view.</param>
		    /// <param name="isDefault" type="Boolean">true, if view will be set as default.</param>
		    var View = { entity: null, view: null, isDefault: false };
		    ...
		    /// <param name="scope" type="Object">The scope for callbacks.</param>
		        ...
		        if (this._views.length > 0) {
		            for (var i in this._views) {
		                ...
		                if (view.isDefault && hadDefault == false) {
		                    this.allowedViews += "@" + entityName + ".@" + view.view; // SetUp initial entity and initial View.
		                    hadDefault = true;
		                }
		                else
		                    this.allowedViews += entityName + "." + view.view;
		            }
		        }
		        else if (this.allowedViews) {
		            /// create suitable fetch for allowed views
		            ...
		            var tmp = this.allowedViews.replace(/([\\&"<>])/g, function (str, item) {
		                return xml_special_to_escaped_one_map[item];
		            });
		            this.allowedViews = "<extra><filter entity='" + this.entities[0] + "'>" + tmp + "</filter></extra>"
		        }

		        var params = { entities: this.entities, source: this.source, prevSelection: this.prevSelection, allowedViews: this.allowedViews, allowNull: this.allowNull };    
